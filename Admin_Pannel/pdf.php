...
$qry = mysqli_query($con, "SELECT * from user_details where ud_id=$uid");
...
$qry2 = mysqli_query($con, "SELECT * from order_details where ud_id=$uid ORDER BY ord_date DESC");
...
{$this->SetFont('Arial', '', 10);
        ...
        $this->Cell(0, 10, "DwarkeshBakers", 0, 1, "C");
        $this->SetFont('Arial', '', 10);
        $this->Cell(0, 10, "Warashia", 0, 1, "C");
...
$total_price = 0;
$i = 1;
$sql1 = "SELECT * FROM order_products WHERE ord_id=".$row2['ord_id'];
...
while ($row1 = mysqli_fetch_array($result1)) {
    ...
    while ($row2 = mysqli_fetch_array($result2)) {
        ...
        $subtotal = $row2["prd_mrp"] * $row1["oprd_qty"];
